...
public class ConvertMetadataController {
    ...
    public Result initOdsLoad(@RequestBody List<EtuInfo> reqParmsEtu){
        ...
        if (!reqParmsEtu.isEmpty()){
            ...
            for (EtuInfo etuEnt:reqParmsEtu){
                ...
                Boolean flag = false;
                // 初始化sqoop字段为null
                String OdsDataSqoopDefine = null;
                // 初始化 hive字段为null
                String OdsDataHivesqlDefine = null;
                // 初始化sqoop脚本信息
                String location = "/home/infa/zwj/ods_import.sh";
                ...
                Statement st = null;
                ResultSet rs = null;
                CjDataSourceConnDefine connDefine = null;
                // 查出所需要的data_base_type
                try {
                    connDefine = iCjDataSourceConnDefineService.
                             selectDataBaseType(etuEnt.getBusinessSystemNameShortName(), etuEnt.getDataSourceSchema());
                } catch (NullPointerException e) {
                    result.error(400,e.toString());
                }
                try {
                    con = GroupPoolFactory.getInstance((etuEnt.getBusinessSystemNameShortName()
                            + (connDefine.getDataBaseType().equals("sqlserver") ? etuEnt.getDataSourceSchema() : "-"))).
                            getConnection();
                    if (con == null) {
                        continue;
                    }
                    st = con.createStatement();
                    // 根据系统缩写，源表名和schema定位数据
                    String executeSql = "select * from datalake_meta.cj_ods_table_load_mode_info where" +
                            " business_system_name_short_name ="+etuEnt.getBusinessSystemNameShortName()+" and " +
                            "data_source_schema ="+etuEnt.getDataSourceSchema()+" and data_source_table ="+etuEnt.getDataSourceTable();
                    rs = st.executeQuery(executeSql);
                    // 创建cj_ods_table_load_mode_info实体类接收ResultSet数据
                    CjOdsTableLoadModeInfo cjOdsTableLoadModeInfo = new CjOdsTableLoadModeInfo();
                    cjOdsTableLoadModeInfo.setBusinessSystemId(rs.getString("BusinessSystemId"));
                    cjOdsTableLoadModeInfo.setBusinessSystemNameShortName(rs.getString("BusinessSystemNameShortName"));
                    cjOdsTableLoadModeInfo.setDataSourceSchema(rs.getString("DataSourceSchema"));
                    cjOdsTableLoadModeInfo.setDataSourceTable(rs.getString("DataSourceTable"));
                    cjOdsTableLoadModeInfo.setOdsDataSchema(rs.getString("OdsDataSchema"));
                    cjOdsTableLoadModeInfo.setOdsDataLoadMode(rs.getString("OdsDataLoadMode"));
                    cjOdsTableLoadModeInfo.setOdsTablePartitionColName(rs.getString("OdsTablePartitionColName"));
                    cjOdsTableLoadModeInfo.setOdsTablePartitionUnit(rs.getString("OdsTablePartitionUnit"));
                    cjOdsTableLoadModeInfo.setOdsTablePrimaryColName(rs.getString("OdsTablePrimaryColName"));
                    cjOdsTableLoadModeInfo.setOdsTableIncrementColName(rs.getString("OdsTableIncrementColName"));
                    cjOdsTableLoadModeInfo.setOdsTableSplitColName(rs.getString("OdsTableSplitColName"));
                    cjOdsTableLoadModeInfo.setOdsTablePartitionColNameSource(rs.getString("OdsTablePartitionColNameSource"));
                    cjOdsTableLoadModeInfo.setLastModifyDt(rs.getString("LastModifyDt"));
                    cjOdsTableLoadModeInfo.setLastModifyBy(rs.getString("LastModifyBy"));

                    // 根据系统缩写，源表名和schema查找cj_ods_table_col_info表，根据ods_table_col_order排序
                   String  tableFieldByOrder = cjOdsTableColInfoService.selectFieldByOrder(etuEnt);
                    SqoopScriptEntity sqoopScriptEntity = new SqoopScriptEntity();
                    sqoopScriptEntity.setFlag(false);
                    sqoopScriptEntity.setLocation(location);
                    sqoopScriptEntity.setUrl(jdbcDriver);
                    sqoopScriptEntity.setUserName(userName);
                    sqoopScriptEntity.setPassWord(passWord);
                    sqoopScriptEntity.setDatabase(cjOdsTableLoadModeInfo.getBusinessSystemNameShortName());
                    sqoopScriptEntity.setSchema(cjOdsTableLoadModeInfo.getDataSourceSchema());
                    sqoopScriptEntity.setTable(cjOdsTableLoadModeInfo.getDataSourceTable());
                    sqoopScriptEntity.setSplitField(cjOdsTableLoadModeInfo.getOdsTableSplitColName());
                    sqoopScriptEntity.setTableField(tableFieldByOrder);
                    OdsDataSqoopDefine = createSqoopInitShell(sqoopScriptEntity);
                    // 分区字段为null则为init_big
                    if (!cjOdsTableLoadModeInfo.getOdsTablePartitionColNameSource().equals(null)){
                        sqoopScriptEntity.setRule("init_big");
                        sqoopScriptEntity.setIncField(cjOdsTableLoadModeInfo.getOdsTablePartitionColNameSource());
                    }else{
                        sqoopScriptEntity.setRule("init");
                        sqoopScriptEntity.setIncField("no");
                    }
                     // 创建CjOdsDataScriptDefInfo实体类
                     CjOdsDataScriptDefInfo cjOdsDataScriptDefInfo = new CjOdsDataScriptDefInfo();
                     cjOdsDataScriptDefInfo.setBusinessSystemId(cjOdsTableLoadModeInfo.getBusinessSystemId());
                     cjOdsDataScriptDefInfo.setBusinessSystemNameShortName(cjOdsTableLoadModeInfo.getBusinessSystemNameShortName());
                     cjOdsDataScriptDefInfo.setDataSourceSchema(cjOdsTableLoadModeInfo.getDataSourceSchema());
                     cjOdsDataScriptDefInfo.setDataSourceTable(cjOdsTableLoadModeInfo.getDataSourceTable());
                     cjOdsDataScriptDefInfo.setOdsDataTable(cjOdsTableLoadModeInfo.getOdsDataTable());
                     cjOdsDataScriptDefInfo.setOdsDataScriptType(cjOdsTableLoadModeInfo.getOdsDataLoadMode());
                     cjOdsDataScriptDefInfo.setOdsDataSqoopDefine(OdsDataSqoopDefine);
                     cjOdsDataScriptDefInfo.setOdsDataHivesqlDefine(OdsDataHivesqlDefine);
                     cjOdsDataScriptDefInfo.setLastModifyDt(FastDateFormat.getInstance("yyyy-MM-dd HH:mm:ss").toString());
                     cjOdsDataScriptDefInfo.setLastModifyBy(cjOdsTableLoadModeInfo.getLastModifyBy());
                     // 持久化实体类
                     cjOdsDataScriptDefInfoService.saveSqoopScripts(cjOdsDataScriptDefInfo);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    /**
     *
     * @param sqoopScriptEntity sqoop生成脚本所需字段数据
     * @return sqoop脚本
     */
    private String createSqoopInitShell(SqoopScriptEntity sqoopScriptEntity){
        // 拼接shell
        StringBuffer sb = new StringBuffer();

        if(sqoopScriptEntity.getFlag() == false){
            sb.append("sh").append(" ").append(sqoopScriptEntity.getLocation()).append(" ").append(sqoopScriptEntity.getUrl()).
                    append(" ").append(sqoopScriptEntity.getUserName()).append(" ").append(sqoopScriptEntity.getPassWord()).
                    append(" ").append(sqoopScriptEntity.getDatabase()).append("~").append(sqoopScriptEntity.getSchema()).append(" ").
                    append(sqoopScriptEntity.getTable()).append(" ").append(sqoopScriptEntity.getIncField()).append(sqoopScriptEntity.getSplitField()).append(" ").
                    append(sqoopScriptEntity.getRule()). append(" ").append("").append("").append(" ").append(sqoopScriptEntity.getTableField());
        }
